...
class tcd_controller_server(Thread):
    ...
    def __init__(self, **kwargs):
        ...
        self.req_header = kwargs.get('req_header', 'tcd_req')
        # Get the reply header from keyword arguments
        self.rep_header = kwargs.get('rep_header', 'tcd_rep')
    ...
    def run(self):
        print('- Started TCD Controller')
        # Run while thread is active
        while not self.shutdown_flag.is_set():
            # Wait for command
            cmd = self.socket.recv_json()
            # TCD SDR request
            tcd_r = cmd.get(self.req_header, None)
            # If the message is valid
            if tcd_r is not None:
                print('- Received Message')
                # Check wheter is it a new radio slice
                nl = tcd_r.get('r_nl', None)
...
if __name__ == "__main__":
    ...
    try:
        ...
        tcd_controller_thread = tcd_controller_server(
